...
async def ban_ip_in_jail(jail_name: str, request_body: IPActionRequest):
    ...
    return ActionResponse(status="success", message=f"IP {request_body.ip_address} baneada.", ip_address=request_body.ip_address, jail=jail_name, command_output=output)
...
async def unban_ip_in_jail(jail_name: str, request_body: IPActionRequest):
    ...
    return ActionResponse(status="success", message=f"IP {request_body.ip_address} desbaneada.", ip_address=request_body.ip_address, jail=jail_name, command_output=output)

@router.get("/jails", response_model=List[str])
async def get_jails():
    output = run_fail2ban_command(["status"])
    line = next((l for l in output.split('\n') if "Jail list:" in l), None)
    if not line:
        if "no jail" in output.lower(): return []
        raise HTTPException(status_code=500, detail="No se pudo obtener la lista de jails")
